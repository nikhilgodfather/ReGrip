...
const InvoiceTyresExcel = ({ checkedData, selectedFleetName }) => {


    const exportToExcelWithoutImages = async () => {
        const workbook = new ExcelJS.Workbook();
        const worksheet = workbook.addWorksheet(`${checkedData[0]?.supplier_name}-${selectedFleetName}_TyreInvoice`);
        let indexNo = 1;
        ...
        checkedData.forEach((item) => {
            const invoiceId = item?.invoice_id ? `INV${item?.invoice_id.toString().padStart(5, '0')}` : "--/--";
        ...
        link.href = blobUrl;
        link.download = `${checkedData[0]?.supplier_name}-${selectedFleetName}_TyreInvoice.xlsx`;
...
export default InvoiceTyresExcel;
